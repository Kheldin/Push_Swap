...
static char	*create_remaining_buffer(char *buf, size_t line_len)
{
	...
	if (ft_strchr_gnl(buf, 10) != -1)
		return (ft_substr_gnl(buf, line_len, ft_strlen_gnl(buf) - line_len));
	res = ft_calloc(ft_strlen_gnl(buf) + BUFFER_SIZE + 1, sizeof(char));
	if (!res)
		return (NULL);
	i = 0;
	while (buf[i])
...
static char	*extract_line(char **buf)
{
	...
	line = ft_substr_gnl(*buf, 0, ft_strchr_gnl(*buf, 10) + 1);
	if (!line)
	{
		free(*buf);
		*buf = NULL;
